...
| **Scope** | The fence decision for a mutation whose impact includes an entity/occupant row that an unresolved StructuralConflict/ConstraintConflict holds at Base. That row only: a contract-specific fence rule, not an entity, parent, ancestor, descendant, or subtree lock. Local delivery is Slices 4–5; sync-origin wiring stays gated on SB-2/OD-1 (Slice 6) and is not decided here |
...
| **ID** | OD-4 |
| **Status** | `CLOSED` 2026-09-14 |
| **Scope** | An S3 ConstraintConflict scope with Base = null, Local absent, and no current occupant, where the owning task (parent) tombstone does not alter the scope's protected occupancy/state. The Constraint policy's contribution for that empty scope only |
...
| **ID** | OD-2 |
| **Status** | `CLOSED` 2026-09-14 |
| **Scope** | Where the fence attaches on the local mutation paths: existing VM/application port → existing repository entry point → T2.4 executor → mutation planning / fence / gates / writer. Sync-origin integration is not in scope (SB-2/OD-1) |
...
| **ID** | OD-7 |
| **Status** | `CLOSED` 2026-09-14 |
| **Scope** | A local save rejected by the structural conflict fence. What happens to a rejected sync-originated mutation (including peer re-offer) is SB-2/OD-1 and is not decided here |
| **Owner ruling** | On a fence rejection of a local mutation: (1) the mutation is **rejected** and **no part** of the rejected save is committed; (2) the in-memory graph is **brought back into consistency with persisted state**; (3) the rejection is **surfaced** to the caller/UI path together with the relevant conflict/rule information; (4) there is **no automatic retry**. Reject ≠ retry; restore/reload ≠ automatic retry |
...
7. No XAML diff. No automatic retry or deferred record after rejection (X-13, N-10). The surfaced rejection carries the relevant conflict/rule information (e.g. the `Blocked` results' rule ids); the channel is Slice-4 engineering.
