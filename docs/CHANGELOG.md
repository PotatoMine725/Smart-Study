...
## 2026-07-05 → 2026-07-19 — Epic 1 (Sync-Ready Data Model) — code complete, release gate in progress (reopened at B4 on 2026-07-15, refixed 2026-07-19)
...
| B4 reopen — R2 (crash visibility) | `Services/CrashLogger` — minimal last-resort fault sink (`%AppData%\SmartStudyPlanner\crash.log`, path overridable for tests; deliberately not a logging framework). `DispatcherUnhandledException` (logs, shows a dialog, sets `Handled = true` so the app survives) / `AppDomain.UnhandledException` / `TaskScheduler.UnobservedTaskException` wired at the top of `OnStartup` — also catches `async void OnStartup` faults, shrinking tech-debt item 2.8-3 without restructuring. The 3 waived fire-and-forget telemetry sites (`MatureAsync`, `LogDifficultyLabelAsync`, `LogWeightChangeAsync`) are now observed, so their faults are logged rather than swallowed (F2 waiver nuance, D-R5) | `b0061e7` + `c18e1e7`, 336 pass |
