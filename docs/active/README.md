...
| Work | Plan | State |
|---|---|---|
| **Prediction instrumentation defect (DFD-9a)** | [`../plans/2026-08-26-prediction-instrumentation-defect.md`](../plans/2026-08-26-prediction-instrumentation-defect.md) | **FIXED 2026-08-26**, suite 487 → 492. Seam returns the prediction record, `TaskDashboardItem` carries it, the write site logs both columns on both branches. **One gate still open:** the end-to-end check (complete a focus session against a real database and read the row back) needs the owner at a keyboard — automated tests cover the three hops but not the production DI wiring |
