...
>
> **Seed upgrade (2026-06-05, post-Slice-5):** the embedded `seed_intents.csv` was regenerated from real data in `datasheets/normalized_dataset_m8a_uniform.csv` (50 hand rows → **596 rows**). The datasheet uses a different label taxonomy `{BaiTap, NhacNho, OnTap, KiemTraThuongXuyen, ThiCuoiKy}`; per the **"remap to enum"** decision we kept only labels with an `LoaiCongViec` home and remapped `BaiTap → BaiTapVeNha`. Dropped `NhacNho`/`OnTap` (no enum). **Consequence:** the seed model now covers **3 of 5** enum classes — `BaiTapVeNha` (216) / `KiemTraThuongXuyen` (188) / `ThiCuoiKy` (192). `ThiGiuaKy` and `DoAnCuoiKy` have **no training rows** (the datasheet labels "đồ án" phrases as BaiTap and "giữa kỳ" as ThiCuoiKy, so hand rows would have *contradicted* the bulk data, not helped) → those two intents fall back to the heuristic parser in Slice 6. `TimeExpression → DeadlineHint`, `Difficulty` kept 1–5. 166 tests still pass.
