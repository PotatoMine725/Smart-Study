...
|---|---|---|---|
| WP-1 | ◐ workflow landed, **gate not yet enforced** | `f98e4c7` (via PR #50) | CI green on `dev` first try — 0 errors, 346 passed on `windows-latest`. No debugging round needed; the tests that touch `%APPDATA%` and build the composition root passed on a fresh runner. **Step 6 closed 2026-08-02 (owner-authorised):** `build-test` is a required check on `dev` and `main`, `strict: true`, force-push and deletion blocked, verified by reading the settings back. The step's stated blocker ("needs admin scope … cannot be done from the CLI") was wrong — the token has admin; the real reason it was an owner action is that it changes how the repo accepts pushes. **Caveat:** `enforce_admins=false`, so the gate does not bind the owner's own direct pushes; WP-1 now enforces for anyone who is not an admin, and still only measures for the one account that is. |
...
| WP-4 | ☑ done | `e89f0ec` | 13 characterization tests, suite 355 → 368. Production untouched (`git diff SmartStudyPlanner/` empty). The plan's 7 draft tests all held against the real implementation and were kept; 6 more were added from reading `WorkloadServiceImpl.cs:40-109` — day-date contiguity across overflow days, the `DiemUuTien` write-back into the caller's model, the exactly-fits chunk-naming boundary, partial `ThoiGianDaHoc`, least-loaded-vs-earliest-with-room placement, and `TenMon` provenance. Non-vacuity verified by mutation: 7 separate mutations of the allocator each turn the suite red. Entry Criterion #7 met. **Found, not fixed:** `capacityHours < 1/60` hangs `GenerateSchedule` in an infinite loop — not UI-reachable (slider is `Minimum="1"` with integer snapping), so it is a `capacity.txt` hazard; flagged to WP-5.2, which owns that file. |
| WP-5 | ☑ done (one manual check outstanding) | `9a175b9` (5.1), `54f64ca` (5.2) | Suite 368 → 379. Report: `docs/reports/2026-08-02-wp5-runtime-robustness.md`. **5.2 closed three defects, not one** — the locale round-trip it was scoped for, plus both WP-4 hand-overs, because all three live in `GetCapacity`. **Two findings beyond the plan.** (a) The plan's "`GetCapacity`/`SaveCapacity` cannot be tested" premise is wrong: it conflates *not injectable* with *not reachable*, and under test `AppDomain.BaseDirectory` is the test assembly's own output dir — writable, not the user profile. 11 tests added, zero production change needed. (b) The `AllowThousands` misparse is **bidirectional**: invariant `"4.5"` read on vi-VN also yields 45, so landing the invariant *write* without the `NumberStyles.Float` *read* would have introduced a 45 h/day capacity on every vi-VN install. Non-vacuity verified by 3 mutations + an unmutated control. `GenerateSchedule` untouched, so WP-4's tests still pin what they pinned. **Deviation from the plan:** the toast cooldown keys on urgency *growth*, not count equality (report §3.4). Entry Criteria **#11 and #12 both met** — #12 is now automated rather than manual, and #11 was verified by the owner on 2026-08-02 (one toast across 12+ ticks at a temporary 10 s interval; pre-fix was one per tick). That takes Epic 2 entry criteria to **9 of 12**; the three left (#1, #8, #10) are owner actions or WP-6. **Found during verification, not fixed:** two independent toast sources (`BackgroundTimer_Tick` and `DashboardViewModel.RaiseNotification`) announce the same urgent task seconds apart — each throttles itself, nothing coordinates them. Documented as report Follow-up 5; a shared throttle is a new seam and the plan's *Out of Scope* excludes those. |
| WP-5 (post-review) | ☑ done | `0e5d448`, `d425068`, `866b5be` | Owner-directed after review. Suite 379 → **391**. `GenerateSchedule` gained the termination guard the package had declined ("untestable without risking a hang" was an *unpriced cost*, not a blocker — a hung CI job is a loud failure); non-vacuity proved by removing the guard and watching the tests **hang**, not go red. Follow-up 5's duplicate toast closed by deleting the dashboard source outright rather than coordinating two. **#11 re-verified 2026-08-02: 1 toast at launch, PASS.** |
| WP-6 | ☑ done | `12291d0` (6.1a), `6416e50` (6.1b + 6.2) | Root `Assets/` retired — design sources committed **before** the delete, so the irreversible `rm -rf` on untracked data was preceded by the data existing in git, not merely by a `diff -r`. README 337 → **391**; roadmap/`CLAUDE.md` GitNexus counts re-measured (4,291/9,848/120 at `12291d0`); `Verify.CommunityToolkit.Mvvm` dropped. Three CSA corrections appended without rewriting the body. **6.3 deliberately not run** — see below. **Two findings.** (a) The plan says criterion #1 needs admin the CLI does not have; the token **does** have admin (`{"admin":true}`), so the stated blocker was wrong — but the real reason it is an owner action stands (it changes how the repo accepts pushes), so it was surfaced, not executed. (b) §8.6's *192 warnings* no longer reproduces: 184 lines / 94 after MSBuild de-duplication. |
| WP-6.3 | ⊘ not run (deliberate) | | Opt-in per the plan, and the owner has not opted in. Independently: its own verification step **requires a manual ML *Retrain*** that an agent cannot perform, so running it would mean landing a dependency change with its stated verification omitted. The advisories stay on the roadmap's deferred ledger. |
...
- [x] **CI runs on every push and PR to `main` and `dev`, the latest run on `dev` is green, and `build-test` is a required status check on `dev`.** *(Met 2026-08-02, owner-authorised. `build-test` is now a required check on **both** `dev` and `main`, with `strict: true` (branch must be current) and force-push and branch-deletion blocked. Verified by reading the settings back with `gh api …/branches/{dev,main}/protection`, not by trusting the write response.*
  *WP-6 correction to the plan's WP-1 Step 6: it claims this "needs admin scope … and cannot be done from the CLI". The token reports `{"admin":true}` and the call succeeded. The reason it stayed an owner action was never the permission — it changes how the repository accepts pushes — and collapsing those two is what made the item look permanently blocked.*
  ***One property this does not yet have.*** `enforce_admins` is **false**, which leaves the owner an escape hatch on a solo repository — and also means the gate does not bind the only account that pushes. Per this repo's own standard, a guard that has not been shown to block anything is not yet demonstrated. The criterion is met **as worded** (the required check is configured); enforcement against the owner needs `enforce_admins=true`, which is a separate workflow decision, not a missing step. See the WP-6 report §3.5.)*
...
- [x] **`WorkloadServiceImpl.GenerateSchedule` has characterization coverage.** Verify: `WorkloadServiceScheduleTests` passes. *(Met at `e89f0ec` — 13 tests, green in Debug and Release. "Passes" alone would be a weak check for a characterization suite, so non-vacuity was verified separately: seven independent mutations of the allocator — most-loaded placement, the overflow offset off-by-one, dropping the completed-task filter, ignoring `ThoiGianDaHoc`, reversing the priority sort, dropping the `DiemUuTien` write-back, and shrinking the 7-day window — each turn the suite red.)*
- [x] **The README's test count equals what `dotnet test` reports.** *(Met at `6416e50`: both read **391**, measured immediately before the edit rather than carried over. The same line's adjacent claim — "the build is clean with zero errors" — was replaced at the same time: literally true, but it sat next to a warning count the CSA had already flagged, so it read as a cleanliness claim the build does not support.)*
...
- [x] **Gate G4 (tombstone retention) is explicitly on the Epic 2 planning agenda.** It is not resolved by stabilization and must not be silently inherited. *(Met at `6416e50`: written into `docs/specs/system_roadmap.md` §A.3 under the LAN-sync entry, where Epic 2 planning will actually look, with the three reasons it is load-bearing — purge divergence resurrects rows on merge, D-I's 3-way merge needs the last-synced base to survive, and unbounded retention grows without limit on a never-syncing device. **Recording it satisfies the criterion as worded; it does not decide it.** The gate is open.)*
- [x] **Deadline toast fires once, not once per tick** (WP-5.1). Manual — no automated coverage exists for `Views/`. **PASSED.** Run with the interval temporarily at 10 s against a real database holding one task scoring 100: exactly **one** background toast across 12+ ticks over several minutes, where the pre-fix behaviour was one per tick. Toast delivery was independently confirmed working (the minimize-to-tray toast fires from an unrelated path), so "one toast" is a genuine observation and not a silent delivery failure. Interval reverted to 5 min and the tree verified identical to HEAD afterwards. Verified by: **Potatomine725 (owner)** on **2026-08-02**.
  **Re-verified after the post-review commits** (`d425068` moved the announcement to launch and deleted the dashboard source; `866b5be` added the once-per-process flag), because the signature above covers only the *timer* path and those commits added an emission point it never saw. Recipe: report §3.5a. Result: **exactly 1 toast at launch — PASS.** `%AppData%\SmartStudyPlanner\crash.log` does not exist, so the scan completed rather than failing into the swallow at `MainWindow.xaml.cs:180`. Re-verified by: **Potatomine725 (owner)** on **2026-08-02**.
  **One anomaly, recorded not dismissed:** on that run — the *first* launch after a Windows reboot — the liveness step failed. Clicking X hid the window but produced no *"đã được thu nhỏ"* toast. It behaved normally on a later launch in the same Windows session. That path (`OnClosing`, `MainWindow.xaml.cs:187-203`) is unconditional, is **untouched by WP-5** (last changed in `c7898be`), and is not part of the deadline-notification mechanism, so it does not qualify the PASS above — the launch toast had already fired minutes earlier from the same process and the same API, which is the delivery proof the liveness step existed to supply. It is carried as a follow-up rather than closed: "works on the second launch" is not the same finding as "works", and first-launch-after-boot is the case a real user meets every morning.
- [x] **`capacity.txt` contains a dot-decimal value that survives a restart** (WP-5.2). **No longer manual** — covered by `WorkloadServiceCapacityTests` (11 tests, `54f64ca`). `GetCapacity` reads the file on every call and holds no cache, so write-then-read-from-disk is the entire round-trip; a restart adds only "is `BaseDirectory` writable in the installed layout". Mutation-verified: reverting the invariant write turns 2 tests red. The 30-second manual check is still worth doing but is no longer the only evidence. Verified by: automated suite on 2026-08-02.
