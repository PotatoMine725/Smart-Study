...
- [x] **CI runs on every push and PR to `main` and `dev`, the latest run on `dev` is green, and `build-test` is a required status check on `dev`.** *(Met 2026-08-02, owner-authorised. `build-test` is now a required check on **both** `dev` and `main`, with `strict: true` (branch must be current) and force-push and branch-deletion blocked. Verified by reading the settings back with `gh api …/branches/{dev,main}/protection`, not by trusting the write response.*
  ...
  ***`enforce_admins` is split by branch, owner-directed.*** `main` = **true** — it is already reached only by PR (#42, #43, #45, #48, #49 are all merges, no direct pushes), so enforcement costs no friction and binds every account including the owner. `dev` = **false** — daily iteration, where all six packages landed by direct push, and there is no second reviewer for a PR ceremony to protect. So the gate is fully binding on released state and advisory on the working branch, by choice. See WP-6 report §3.5a–b.)*
