...
- [ ] **Deadline toast fires once, not once per tick** (WP-5.1). Manual — no automated coverage exists for `Views/`. **Still outstanding.** Needs a live app, a task scoring ≥80, and a human watching Windows notifications; the agent did not run it and therefore did not sign it. Discriminating recipe (interval temporarily at 10 s → expect exactly one toast in ~60 s, where the pre-fix behaviour was six) in `docs/reports/2026-08-02-wp5-runtime-robustness.md` §3.5. Verified by: ______ on ______.
