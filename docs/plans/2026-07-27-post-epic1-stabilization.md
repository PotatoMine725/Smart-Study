...
- [ ] **CI runs on every push and PR to `main` and `dev`, the latest run on `dev` is green, and `build-test` is a required status check on `dev`.** Verify: `rtk gh run list --limit 5` shows `completed success`; the required-check setting is visible in *Settings → Branches* (owner action — see WP-1 Step 6). **The only criterion still open.** Halves 1 and 2 are met — the workflow runs on every push and PR and `dev` is green. Half 3 is not: `gh api repos/:owner/:repo/branches/dev/protection` returns **404 Branch not protected**. *(WP-6 correction: the plan says this needs admin scope the CLI lacks. It does not — the token reports `{"admin":true}`, so it is executable from here. It was still not executed: required status checks reject direct pushes, which changes how the owner works day to day, and that is a policy call rather than a missing permission. Surfaced for the owner with the exact API call ready.)*
